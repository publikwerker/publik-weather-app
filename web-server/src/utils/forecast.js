...
const forecast = (data, callback) => {
  ...
  request({url, json: true}, (err, {body}) => {
    if (err){
      ...
      callback(undefined, chalk.yellow(`The temperature in ${place}, is ${temperature} degrees Fahrenheit, with ${precip}% chance of rain. The forecast calls for ${summary.toLowerCase()}`)
      );
    };
  });
};
